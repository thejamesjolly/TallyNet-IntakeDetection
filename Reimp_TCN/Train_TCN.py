...
if False:
    print("TESTING MODEL INTIALIZATION WITH FICTITIOUS DATA")
    save_model=False # Turn off save features
    MODEL_ARCH_FLAG = 1

    # Test Model
    if MODEL_ARCH_FLAG == 0: # flatten model outputs  
        num_axes = 6
        Data_Freq = 16 # Hz
        CUT_sec = 60.0
        ...
        raise ValueError("Invalid MODEL_ARCH_FLAG Value.")
    # end
    ...
    if MODEL_ARCH_FLAG == 0:
        myModel = Create_TCN_Classifier(sample_length, total_axes)
    elif MODEL_ARCH_FLAG == 1:
    ...
    print("Number of parameters in TCN Reimplementation model:", get_n_params(model))
...
if True:
    ...
    # Default Values
    RESAMPLE_FLAG_FREQ = 16 # Match models intial frequency for same temporal scope  
    ...
    MODEL_ARCH_FLAG = 1 # Value to determine mode of TCN Reimplmentation
            ...
            # Hardcoded values Seq2One with exact size: narrow tensor afer TCN to single timestep
			# 1 = 1023 sequence length (60 sec at 16 Hz)
			# 2 = 511 sequence length (30 sec at 16 Hz)
			# 3 = 256 sequence length (15 sec at 16 Hz)
            # FUTURE OPTION = Seq2Seq 
    ...
    print('MODEL_ARCH_FLAG Input is ', MODEL_ARCH_FLAG)
    ...
    print('TCN Depth is {}'.format(TCN_Depth))
