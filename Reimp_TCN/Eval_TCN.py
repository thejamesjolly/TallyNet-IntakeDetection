'''
Filename: Eval_TCN.py
Edited By: James Jolly

Version Record
0.2 - April 27, 2026
    Added Create_TCN_Seq2OneClassifier, which collapses TCN output to single midpoint timestep
0.1 - April 15, 2026
    Original Attempt: Flattens entire sequence into a single output. Very high paramter count
...
#import matplotlib.pyplot as plt
from TCN_Classifier import Create_TCN_Classifier
from TCN_Classifier import Create_TCN_Seq2OneClassifier
...
BATCH_SIZE = 64 # EVAL BATCH SIZE # Limit due to large input windows of TCN Model
...
if __name__ == '__main__':
    ...
    CUT_sec = 4.0 # default window size
    RESAMPLE_FLAG_FREQ = 16 # zero if keeping original data frequency, new freq otherwise
    ...
    TCN_Depth = 6 # original paper uses 9
    ...
    LABEL_OFFSET = 0.5 # Using non-causal TCN, so point-wise label should be taken from the middle of the window 
    ...
    MODEL_ARCH_FLAG = 1 # Value to determine mode of TCN Reimplmentation
            # 0 = first attempt: Flatten all timesteps to single output
            # 1 = Seq2One with exact size: narrow tensor afer TCN to single timestep
            # FUTURE OPTION 2 = Seq2Seq:
    ...
    if len(sys.argv) > NUM_REQUIRED_INPUTS:
        ...
        while i < len(sys.argv):
            ...
            if opt_arg == '--stride':
                ...
                CUT_sec = float(sys.argv[i+1])
            elif opt_arg == '--arch':
                MODEL_ARCH_FLAG = int(sys.argv[i+1])
            elif opt_arg == '--tcn-depth':
                TCN_Depth = int(sys.argv[i+1])
            elif opt_arg == '--resamp':
                ...
                LABEL_OFFSET = float(sys.argv[i+1])
    ...
    # if architecture is hardcoded sequences 
    if MODEL_ARCH_FLAG >= 1:
        if DEBUG_PRINTS == 1:
            print("Using predefined values for CUT_sec and TCN_Depth model architecture...")
        # end of DEBUG_PRINTS
        LABEL_OFFSET = 0.5 # always midpoint 
        if MODEL_ARCH_FLAG == 1:
            CUT_sec = 1023.0 / RESAMPLE_FLAG_FREQ # CUT = 1023 
            TCN_Depth = 9
        elif MODEL_ARCH_FLAG == 2: 
            CUT_sec = 511 / RESAMPLE_FLAG_FREQ # CUT = 511
            TCN_Depth = 8
        # end switch MODEL_ARCH_FLAG
    # end if using predefined Seq2One architecture

    
    DATA_FREQ = 16 if (RESAMPLE_FLAG_FREQ == 0) else RESAMPLE_FLAG_FREQ # TCN paper orignially at 16 Hz
    ...
    if DEBUG_PRINTS == 1:
        ...
        print('LABEL_OFFSET (if needed) is ',LABEL_OFFSET)
    ...
    sample_data = np.array(features_input[0:8]).astype(np.float32) # convert to f32 for pytorch
    ...
    if MODEL_ARCH_FLAG == 0:
        myModel = Create_TCN_Classifier(sample_length, total_axes, TCN_Depth)
    elif MODEL_ARCH_FLAG == 1:
        myModel = Create_TCN_Seq2OneClassifier(total_axes, MODEL_ARCH_FLAG = MODEL_ARCH_FLAG)
        ...
        if DETECTION_METHOD_FLAG == 1: # First Method: Floor predictions, then find any local maximum (based on its two neighbors), 
            ...
            for i in range(len(predictions_LM)):
                ...
                if predictions_LM[i]==1:
                    ...
                    Detections.append(timesteps[running_max_index] + LABEL_OFFSET * CUT_sec) # Offset by percentage of the window specified by LABEL_OFFSET
                ...
                if predictions[rover1]==0: #if no detection, move to next bite
                    ...
                    for rover3 in range(rover1+1,rover2): # iterate over all remaining data in segment
                        if predictions[rover3]>=currMax: # biased to later times if a tie
                            ...
                            currMaxIndex=rover3
                    Detections.append(timesteps[currMaxIndex]+LABEL_OFFSET * CUT_sec)
    ...
    for EvalMethod in EVAL_METHODS_TO_USE:
        ...
        for curr_meal in range(len(compiled_eval_data)):	
            ...
            Detections[Detections<0] = 0.00001 # Replace all negative values with small positive number
