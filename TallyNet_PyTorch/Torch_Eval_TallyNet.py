...
log_interval=50 #how many batches to wait before logging training status (default: 10) 
...
if __name__ == '__main__':
    ...
    MODEL_ARCH_FLAG = 3001 # Default to PyTorch A1333 Architecture
    ...
    if MODEL_ARCH_FLAG == 4211: # if using hard coded input for TCN
        if DEBUG_PRINTS == 1:
        ...
        CUT_sec = 1023.0 / DataFreq # hardcode 1023 sequence length
    ...
    # Only load a small amount of samples to reduce memory cost
        ...
        if SAVE_PREDICITONS_FLAG == 1:
            ...
            with open(csvfilename,'w') as fp:
                for i in range(len(predictions)):
                    fp.write("{:.4f},{:.4f}\n".format(predictions[i],timestep_ref[i]))
                # end of for loop
            #end of with fp
