'''
Filename: Train_GRU.py
Author: James Jolly

Version Record
0.1 - April 28, 2026
    Original Attempt: Reimplementation of CNN-GRU-MHA arhcitecture from the paper
    "Explainable CNN-GRU Model With Self-Attention for Human Activity Recognition
    Using Wearable Sensors Through Data Augmentation" by Nadia Sultana et al.
...
from GRU_Classifier import Create_GRU_Classifier
...
def train_and_test(model, save_name="default_saved_model.pt", epochs=5):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (epoch % checkpoint_interval == 0):
            print("Saving Checkpoint model at epoch {}...".format(epoch), flush=True)
...
if False: # DEBUG MODEL TESTING
    ...
    if MODEL_ARCH_FLAG == 0: # flatten model outputs  
        num_axes = 6
        Data_Freq = 15 # Hz
        CUT_sec = 10 # Paper tested 5, 10, 20 sec window; 10 sec recommended
        sequence_length = int(round(CUT_sec * Data_Freq))  # 5.12 seconds @ 100 Hz
        sample_length = sequence_length
        batch_size = 64 # rough reccomendation from paper
    else:
    ...
    if MODEL_ARCH_FLAG == 0:
        myModel = Create_GRU_Classifier(sample_length, total_axes)
    ...
    # for name,param
    print("\nTotal Number of parameters in TCN Reimplementation model:", get_n_params(model))
...
# if __name__ == '__main__':
    ...
    LABEL_OFFSET = 1.0 # Default to the end of the current window 
    
    MODEL_ARCH_FLAG = 0 # Value to determine mode of TCN Reimplmentation
            # 0 = Upper param count of the OG Paper, Default values
            # 1 = Lower end of the OG paper param count 
            # 2 = Large Param (128, 256, 128)
    ...
    print("python setup complete", flush=True)
    ...
    myModel = Create_GRU_Classifier(sample_length, total_axes, MODEL_ARCH_FLAG = MODEL_ARCH_FLAG)
