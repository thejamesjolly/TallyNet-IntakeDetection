...
class Dataset():
  ...
  def ids(self):
    ## ORIG ### JPJ REMOVE EXTRA STRUCTURE IN CLEMSON STORAGE
  ...
  def check(self, id):
    ...
    gesture_dir = os.path.join(self.src_dir, "Data", id[0], id[1], "gesture_union.txt")
    ### JPJ REPLACE FILE PATH TO DEFAULT STRUCTURE
    ...
    # bite_dir = os.path.join(self.src_dir, "all-gt-bites", id[0],
      # id[1], "gt_union.txt")
    bite_dir = os.path.join(self.src_dir, "Data", id[0], id[1], "gt_union.txt")
    ### JPJ REPLACE FILE PATH TO DEFAULT STRUCTURE
  ...
  def data(self, _, id):
    ...
    files = glob.glob(os.path.join(dir, "2*.txt"))
      ...
      with tf.io.TFRecordWriter(path) as tfrecord_writer:
        for i in range(0, len(timestamps)):
          example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'example/dom_acc': _floats_feature(acc[i].ravel()),
            'example/dom_gyro': _floats_feature(gyro[i].ravel()),
