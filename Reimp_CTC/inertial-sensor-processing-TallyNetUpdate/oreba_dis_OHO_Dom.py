"""OREBA-DIS One Hand dataset"""
"""
  Dom - Modified by James Jolly to Only Output Dominant Hand Data
        with Dominant GT labels
  Dnd - Modified by James Jolly to Only Output Dominant Hand Data
        with Dominant AND Non-Dominant GT labels
"""
...
class Dataset():
  ...
  def data(self, _, id):
    logging.info("JPJ Reading PROCESSED data from csv")
    file = os.path.join(self.src_dir, id, id + "_inertial_processed.csv")
    assert file, "JPJ No processed data found for {}".format(id)
    dom_acc, dom_gyro, ndom_acc, ndom_gyro = [], [], [], []
    with open(file) as dest_f:
      reader = csv.reader(dest_f, delimiter=",")
      next(reader) # Skip header
      # Read values
      for row in reader:
        dom_acc.append([float(row[3]), float(row[4]), float(row[5])])
        dom_gyro.append([float(row[6]), float(row[7]), float(row[8])])
        ndom_acc.append([float(row[9]), float(row[10]), float(row[11])])
        ndom_gyro.append([float(row[12]), float(row[13]), float(row[14])])
    # Derive timestamps
    count = len(dom_acc)
    dt = TIME_FACTOR // FREQUENCY # In microseconds
    timestamps = range(0, count*dt, dt)
    return timestamps, {"dom":  (dom_acc, dom_gyro), \
              "ndom": (ndom_acc, ndom_gyro)}
  ...
  def labels(self, _, id, timestamps):
    domHand=(self.dominant(id)).lower()
    ...
    with open(path) as dest_f:
      ...
      for row in csv.reader(dest_f, delimiter=','):
        #JPJ V3 Edit: Only grab Dominant Hand Gesture Labels
        if (row[6].lower() == domHand) or (row[6].lower() == 'both'):
          numDomIntakes+=1
        ...
        # end of if dom hand intake
      # end of for row in csv
          logging.info("JPJ Number of dominant hand intake gestures = {}...".format(numDomIntakes)) 
    # Update class names
  ...
  def write(self, path, id, timestamps, data, dominant_hand, labels):
    ...
    dom_acc = np.asarray(data["dom"][0])
    dom_gyro = np.asarray(data["dom"][1])
    ndom_acc = np.asarray(data["ndom"][0])
    ndom_gyro = np.asarray(data["ndom"][1])
    assert len(timestamps) == len(dom_acc), \
      "Number timestamps and acc readings must be equal"
    assert len(timestamps) == len(ndom_acc), \
      "Number timestamps and acc readings must be equal"
    assert len(timestamps) == len(dom_gyro), \
      "Number timestamps and acc readings must be equal"
    assert len(timestamps) == len(ndom_gyro), \
      "Number timestamps and acc readings must be equal"
        ...
        if self.exp_uniform:
          ...
          for i in range(0, len(timestamps)):
            writer.writerow([id, frame_ids[i], timestamps[i],
              dom_acc[i][0], dom_acc[i][1], dom_acc[i][2],
              dom_gyro[i][0], dom_gyro[i][1], dom_gyro[i][2],
        ...
        for i in range(0, len(timestamps)):
          if self.exp_uniform:
            example = tf.train.Example(features=tf.train.Features(feature={
              ...
              'example/dom_acc': _floats_feature(dom_acc[i].ravel()),
              'example/dom_gyro': _floats_feature(dom_gyro[i].ravel()),
