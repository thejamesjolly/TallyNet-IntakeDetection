...
echo "Evaluating Clemson model "$ModelIdentifer" Fold "$FoldNum" and Checkpoint "$CheckpointID"..."
echo "Using BOTH Dominant and Non-Dominant Hand Intakes from ClemsonDND Data..."

# Uses OREBA recommended split
python main.py --dataset=clemson --input_length=120 --seq_shift=2 --model_dir=DNDClem_CTC_"$ModelIdentifier"/Fold"$FoldNum" --mode=predict --predict_dir=Preds_DNDClem_CTC_"$ModelIdentifier"/Fold"$FoldNum"_"$EvalID" --eval_dir=/project/ahoover/mhealth/jpjolly/CTC_reimp_data/ClemsonDND/Fold"$FoldNum"/eval --model_ckpt="$CheckpointID" --use_def=False --beam_width=3 
