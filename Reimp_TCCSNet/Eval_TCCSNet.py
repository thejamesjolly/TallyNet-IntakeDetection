...
if __name__ == '__main__':
    ...
    MODEL_ARCH_FLAG = 1 # 1 = CSNet, 2 = TCCSNet
    STRIDE_sec = 1 # The time to move between each window when making examples in [sec]
    CUT_sec = 4.0 # default window size
    RESAMPLE_FLAG_FREQ = 32 # zero if keeping original data frequency, new freq otherwise
    ...
    DATA_FREQ = 32 if (RESAMPLE_FLAG_FREQ == 0) else RESAMPLE_FLAG_FREQ # TCCSNet Model Set at 32 Hz
        ...
        with torch.no_grad():
            for data in meal_loader:
                ...
                if firstPassFlag == True: # define predictions on first pass
                    predictions = torch.sigmoid(output.squeeze(dim=1))
                    firstPassFlag = False
                else: # append if second pass
                    predictions_part = torch.sigmoid(output.squeeze(dim=1))
