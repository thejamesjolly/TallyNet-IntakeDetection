...
if __name__ == '__main__':
	...
	for idx in range(len(AllAnnotationFiles)):
		currFileName=AllAnnotationFiles[idx]
		...
		with open(currFileName) as currFile:
			...
			for line in currFile:
				row2=line.split(',')
				bite_hand = row2[6].lower()
				if (currDomHand == bite_hand) or (bite_hand == 'both'): # Only record Dominant Hand Bites
					if bite_hand == 'right':
				...
				# end of if DomHand is taking Bite
	...
	outfile = open("../Dom_OneHandOreba.pkl",'wb')
