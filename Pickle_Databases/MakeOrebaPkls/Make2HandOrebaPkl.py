...
Author: James Jolly
Purpose: 
	Create a pickle file of the Two Hand OREBA dataset 
...
if __name__ == '__main__':
	...
	for currFileName in AllDataFiles:
		cntr = cntr+1
		if (cntr % 10 == 0):
			print("Through {:d} participant files...".format(cntr))
		#end of if
	...
	README = [["INTRODUCTION",
			   'This pkl dataset contains all data from the OREBA (Both Hands) Dataset processed for easy use in Python in IMU event tracking.'
			  ],
			  ["DICTIONARY ENTRIES IN DATABASE",
			   ...
			   'proc_data: Sensor readings of the dominant hand as reported in OREBA, processed to be standardized/normalized, gravity removed, and mirroring all left handed meals to right handed. Given as acc_x, acc_y, acc_z, then gyro_x, gyro_y, gyro_z for the dominant hand, then the same order of measurements for the non-dominant hand. Due to standardization, all values are unitless.',
	...
	outfile = open("../Dnd_TwoHandOreba.pkl",'wb')
